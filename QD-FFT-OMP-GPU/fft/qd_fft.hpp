...
void dif(uint64_t N, uint64_t n, double *x[], double *ix[], double *y[], double *iy[], qd cos_table[], qd sin_table[]) {
    if (n <= 1)
        return;
...
void inv_dif(uint64_t N, uint64_t n, double *x[], double *ix[], double *y[], double *iy[], qd cos_table[], qd sin_table[]) {
    if (n <= 1) {
        div_pwr2(x[0], N, y[0]);
...
// inline void stockham_for_sixstep(uint64_t n, uint64_t p, uint64_t u, qd* x, qd** y, qd cos_table[], qd sin_table[]) {
//     qd **x0     = x;
//     qd **x1     = y;
//     uint64_t l = n >> 1;
//     uint64_t m = 1;
...
//             double *b = (double *)sin_table[j * n / (2 * l) * u];
//             for (uint64_t k = 0; k < m; k++) {
//                 double *c0  = (double *)x0[0][k + j * m];
//                 double *ic0 = (double *)x0[1][k + j * m];

//                 double *c1  = (double *)x0[0][k + j * m + l * m];
//                 double *ic1 = (double *)x0[1][k + j * m + l * m];

//                 double *xt1  = (double *)x1[0][k + 2 * j * m + m];
//                 double *ixt1 = (double *)x1[1][k + 2 * j * m + m];

//                 // c0 + c1
//                 // x1[k + 2jm] <- c0 + c1
//                 add(c0, c1, x1[0][k + 2 * j * m]);
//                 // ix1[k + 2jm] <- ic0 + ic1
//                 add(ic0, ic1, x1[0][k + 2 * j * m]);
...
// void sixstep_fft_by_real_fft(uint64_t N, uint64_t logN, qd** x, qd** y, qd cos_table[], qd sin_table[]) {
//     uint64_t n1 = 1 << (logN / 2);
//     uint64_t n2 = 1 << ((logN + 1) / 2);
//     uint64_t u1 = N / n1;
//     uint64_t u2 = N / n2;
...
//     #pragma omp target data map(to:x[:2][:N], cos_table[:N], sin_table[:N]) map(from:y[:2][:N])
//     {
//         #pragma omp target teams distribute parallel for collapse(3)
//         for (uint64_t k = 0;k < 2;k++) {
//             for (uint64_t i = 0;i < n1;i++) {
//                 for (uint64_t j = 0;j < n2;j++) {
//                     copy(x[k][i * n2 + j], y[k][j * n1 + i]);
//                 }
//             }
//         }
