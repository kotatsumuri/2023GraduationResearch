...
inline void fft_even(uint64_t n, uint64_t p, qd *x, qd *ix, qd *y, qd *iy, qd w[]) {
    uint64_t l = n >> 1;
    uint64_t m = 1;
...
inline void fft_odd(uint64_t n, uint64_t p, qd *x, qd *ix, qd y[], qd iy[], qd w[]) {
    uint64_t l = n >> 1;
    uint64_t m = 1;
#pragma omp target data map(tofrom : x[ : n], ix[ : n]) map(to : w[ : n / 4 + 1]) map(alloc : y[ : n], iy[ : n])
    {
...
void stockham_gpu_n4(uint64_t n, uint64_t p, qd *x[], qd *ix[], qd w[], Timer &timer) {
    qd *y  = (qd *)calloc(n, sizeof(qd));
    qd *iy = (qd *)calloc(n, sizeof(qd));
    timer.start();
    if (p & 1)
