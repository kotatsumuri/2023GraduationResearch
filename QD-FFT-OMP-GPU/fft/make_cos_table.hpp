...
void make_quater_cos_table(uint32_t n, double cos_table[]) {
    init(cos_table, 1.0);
    if (n <= 2)
        return;
...
void make_cos_table(uint32_t n, double cos_table[]) {
...
void make_sin_table(uint32_t n, double sin_table[], double cos_table[]) {
    if (n == 2) {
        init(sin_table, 0);
        init(sin_table + 4, 0);
        return;
