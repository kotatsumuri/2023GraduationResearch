...
#pragma omp parallel for
        ...
        stockham_gpu(n, p, &x, &ix, w, iw);

        for (uint32_t i = 1; i < (1 << 30); i <<= 1) {
            double min_elapsed_time = 1e10;
            double min_h2d_time, min_d2h_time, min_kernel_time;
            for (uint32_t j = 0; j < 8; j++) {
                ...
                if (timer.elapsed_time() < min_elapsed_time) {
                    ...
                    min_h2d_time     = h2d_timer.elapsed_time();
                    min_d2h_time     = d2h_timer.elapsed_time();
                    min_kernel_time  = kernel_timer.elapsed_time();
                }
            ...
            if (min_elapsed_time > THRESHOLD) {
                ...
                std::cout << ", " << min_h2d_time / i;
                std::cout << ", " << min_d2h_time / i;
                std::cout << ", " << min_kernel_time / i;
                std::cout << ", " << (min_elapsed_time - min_h2d_time - min_d2h_time - min_kernel_time) / i;
                std::cout << std::endl;
                break;
            }
        }
