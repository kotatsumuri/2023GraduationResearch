...
int main(int argc, char *argv[]) {
    Arg arg          = Arg(argc, argv, 4, {"--debug", "--cos-real", "--cos-imag", "--range"}, {}, {});
    ...
    std::cout << "thread_num, n, real-average-error-bit, imag-average-error-bit, average-error-bit" << std::endl;
    for (uint64_t n = start_n, p = start_p; n <= end_n; n <<= 1, p++) {
        ...
        for (uint64_t thread_idx = 0; thread_idx < atoi(arg._argv[2]); thread_idx++) {
            uint64_t thread_num = atoi(arg._argv[3 + thread_idx]);
            ...
            sixstep_omp(n, p, &x, &ix, w, iw);
            if (is_debug) {
                std::cout << "sixstep" << std::endl;
                std::cout << "x" << std::endl;
                print_vector(n, x);
                std::cout << "ix" << std::endl;
                print_vector(n, ix);
            ...
            inv_sixstep_omp(n, p, &x, &ix, w, iw);
            if (is_debug) {
                std::cout << "inv_sixstep" << std::endl;
                std::cout << "x" << std::endl;
                print_vector(n, x);
                std::cout << "ix" << std::endl;
                print_vector(n, ix);
